...
class ProjectService:
  ...
  async def get_projects(
    ...
    if params.order == "desc" and order:
      order = "-" + order
    if order:
    ...
    if params.ids:
      if order is None:
        ...
        return PaginatedData(
          ...
          pages=(len(results) + params.page_size - 1) // params.page_size,
        )
      else:
        query = (
          Project.filter(id__in=params.ids).order_by(order).prefetch_related("tags")
        )
