...
class NotificationService:
  ...
  async def get_notifications(user_id: int):
    notifications = await Notification.filter(user_id=user_id).order_by('-created_at').all().prefetch_related(
        Prefetch(
  ...
  @staticmethod
  async def notify_admins(message: str, related_project: int = None, related_comment: int = None):
    admins = await User.filter(role=Role.ADMIN).all()
    for admin in admins:
      await Notification.create(user=admin, content=message, related_project_id=related_project, related_comment_id=related_comment)

  @staticmethod
  async def notify_user(message: str, user_id: int, related_project: int = None, related_comment: int = None):
    await Notification.create(user_id=user_id, content=message, related_project_id=related_project, related_comment_id=related_comment)
